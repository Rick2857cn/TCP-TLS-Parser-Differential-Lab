...
为了在本实验中构造一个确实能穿过 Naive 版本的对照案例，实验同时制造两个不同视图：

```text
策略中的黑名单地址：192.0.2.10
输入的等价表示：    ::ffff:192.0.2.10
SNI 数据块：         blocked. | test
```

Naive 版本没有把 IPv4 映射 IPv6 地址还原为 IPv4，因此漏掉 IP；它又分别解析两个不完整的 ClientHello 数据块，因此漏掉 SNI。两个条件都被错误判为未命中，后端最终收到完整的 `blocked.test` ClientHello。Reassembly 版本先把映射地址规范化为 `192.0.2.10`，立即得到 BLOCK；即使没有 IP 命中，重组后的 SNI 检查也会阻断。

这不是客户端在真实连接中任意改写目标 IP：映射地址仍由本地实验运行器作为策略元数据提供，全部 socket 继续使用 `127.0.0.1`。该结果只说明，当 IP 规范化和 TLS 重组同时存在实现缺陷时，`IP OR SNI` 策略也可能被两个漏判组合穿过；对连接上下文中的真实地址先做规范化的防火墙不受这个案例影响。
